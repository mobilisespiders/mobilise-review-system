...
function Toast({ message, type, onClose }) {
  ...
  const colors = {
    ...
    info:    { bg: "linear-gradient(135deg, #0A919B, #e2e2e0)", icon: "i" },
  };
  ...
  return (
    <div style={{
      ...
      borderRadius: 12, boxShadow: "0 8px 32px rgba(32,38,38,0.14)",
      display: "flex", alignItems: "center", gap: 12,
      animation: "slideIn 0.4s ease", minWidth: 280, maxWidth: 420,
      fontFamily: "'Inter', 'Segoe UI', sans-serif",
    }}>
      <span style={{ fontSize: 18, fontWeight: 500, width: 28, height: 28,
...
function Dashboard() {
  ...
  const assignReviews = async () => {
    ...
    if (assignmentsPerUserValue > maxAssignmentsPerUser) {
  ...
  const startEditAssignments = (reviewerId) => {
    if (!selectedBatchId) {
      showToast("Select a batch before editing assignments", "error");
      return;
    }

    const currentReviewees = assignments
      .filter(a => String(a.batch_id) === String(selectedBatchId) && String(a.reviewer_id) === String(reviewerId))
      .map(a => a.reviewee_id);

    setEditingReviewerId(reviewerId);
    setEditRevieweeIds([...new Set(currentReviewees)]);
  };
  ...
  const toggleEditReviewee = (userId) => {
    if (String(userId) === String(editingReviewerId)) {
      showToast("Reviewer cannot review themselves", "error");
      return;
    }

    setEditRevieweeIds(prev =>
  ...
  const saveReviewerAssignments = async () => {
    if (!selectedBatchId || !editingReviewerId) {
      showToast("Select a batch and reviewer before saving", "error");
      return;
    }

    if (editRevieweeIds.length === 0) {
      showToast("Select at least one reviewee", "error");
      return;
    }
    ...
    try {
      const res = await axios.put(
        `${BASE_URL}/assignment-batches/${selectedBatchId}/reviewers/${editingReviewerId}/assignments`,
        { reviewee_ids: editRevieweeIds }
      );
      showToast(`${res.data.assignments_updated} assignments saved`, "success");
      await fetchAssignments(selectedBatchId);
      cancelEditAssignments();
    } catch (error) {
      console.error(error);
      showToast(error.response?.data?.detail || "Failed to save assignments", "error");
    } finally {
  ...
  const toggleRecipient = (userId) => {
    setManualReviewer(prev =>
      prev.includes(userId)
        ? prev.filter(id => id !== userId)
        : manualReviewee.includes(userId)
          ? prev
          : [...prev, userId]
    );
    if (!manualReviewer.includes(userId) && manualReviewee.includes(userId)) {
      showToast("A user cannot be both reviewer and reviewee", "error");
    }
  };
  ...
  const toggleAssignee = (userId) => {
    setManualAssignees(prev =>
      prev.includes(userId)
        ? prev.filter(id => id !== userId)
        : manualReviewer.includes(userId)
          ? prev
          : [...prev, userId]
    );
    if (!manualReviewee.includes(userId) && manualReviewer.includes(userId)) {
      showToast("A user cannot be both reviewer and reviewee", "error");
    }
  };
  ...
  const selectAllReviewer = () => {
    const filtered = users.filter(u =>
      ...
      !manualReviewee.includes(u.user_id)
    );
    if (filtered.length === 0) {
      showToast("No eligible reviewers in this list", "error");
      return;
    }
  ...
  const selectAllAssignees = () => {
    const filtered = users.filter(u =>
      ...
      !manualReviewer.includes(u.user_id)
    );
    if (filtered.length === 0) {
      showToast("No eligible reviewees in this list", "error");
      return;
    }
  ...
  const sendManualAssignment = async () => {
    ...
    if (manualReviewer.some(id => manualReviewee.includes(id))) {
      showToast("Remove users selected as both reviewer and reviewee", "error");
      return;
    }
    ...
    try {
      ...
      if (res.data.batch_id) {
        ...
        fetchAssignments(res.data.batch_id);
      } else {
        fetchAssignments();
      }
      // Reset
  ...
  const filteredAssignments = assignments.filter(a => {
    let match = true;
    if (selectedBatchId && String(a.batch_id) !== String(selectedBatchId)) match = false;
  ...
  const assignmentGroups = Object.values(filteredAssignments.reduce((groups, assignment) => {
    const reviewerId = assignment.reviewer_id;
    if (!groups[reviewerId]) {
      groups[reviewerId] = {
        reviewer_id: reviewerId,
        reviewees: [],
      };
    }
    groups[reviewerId].reviewees.push(assignment.reviewee_id);
    return groups;
    ...
    return (
      <div style={{
        ...
        <style>{`
          ...
          input:focus { border-color: #0A919B !important; box-shadow: 0 0 0 3px rgba(10,145,155,0.14) !important; }
        `}</style>
        <div style={{
          width: "100%",
          maxWidth: 420,
          ...
          <div style={{
            ...
            <div style={{ width: 46, height: 46, borderRadius: 12, background: "rgba(255,255,255,0.16)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 500, fontSize: 18 }}>
              M
            </div>
            <div>
              <h1 style={{ margin: 0, fontSize: 22, fontWeight: 500 }}>MTI Admin</h1>
          ...
          <form onSubmit={handleAdminSignIn} style={{ padding: 28, display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <label style={{ fontSize: 12, fontWeight: 500, color: "#000000", textTransform: "uppercase", letterSpacing: 0.5 }}>User ID</label>
              <input
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <label style={{ fontSize: 12, fontWeight: 500, color: "#000000", textTransform: "uppercase", letterSpacing: 0.5 }}>Password</label>
              <input
                type="password"
            ...
            {adminLoginError && (
              <div style={{ background: "#fef2f2", color: "#dc2626", border: "1px solid #fecaca", borderRadius: 10, padding: "10px 12px", fontSize: 13, fontWeight: 500 }}>
                {adminLoginError}
              </div>
            )}
            ...
            <button
              ...
              style={{
                ...
                fontWeight: 500,
                fontSize: 14,
                boxShadow: "0 8px 22px rgba(10,145,155,0.24)",
              }}
            >
              Sign In
            </button>
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "linear-gradient(135deg, #ffffff 0%, #e2e2e0 100%)", fontFamily: "'Inter', 'Segoe UI', sans-serif" }}>
      {/* Global Animations */}
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap');
        ...
        .check-item:hover { background: #ffffff !important; }
        input:focus, select:focus { border-color: #0A919B !important; box-shadow: 0 0 0 3px rgba(10,145,155,0.14) !important; }
      `}</style>

      {/* Sidebar */}
      <div style={{
        width: 260, background: "linear-gradient(180deg, #12484c 0%, #0A919B 50%, #0e2931 100%)",
      ...
      <div style={{ flex: 1, padding: "28px 36px", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
          <div>
            <h1 style={{ fontSize: 26, fontWeight: 500, color: "#000000", margin: 0 }}>Admin Dashboard</h1>
            <p style={{ color: "rgba(0,0,0,0.58)", fontSize: 13, marginTop: 4 }}>{currentTime.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
          </div>
        ...
        {activeTab === 'Welcome' && (
          <div style={{ animation: "fadeUp 0.5s ease" }}>
            <div className="card-hover" style={{
              ...
              minHeight: 300, background: "linear-gradient(135deg, #ffffff 0%, #ffffff 100%)",
              borderRadius: 20, boxShadow: "0 4px 24px rgba(0,0,0,0.06)", padding: 48,
              textAlign: "center", border: "1px solid rgba(0,0,0,0.06)",
            }}>
              <h2 style={{ fontSize: 32, fontWeight: 500, color: "#000000", marginBottom: 8 }}>{getGreeting()}, Admin!</h2>
              <p style={{ fontSize: 16, color: "rgba(0,0,0,0.58)", maxWidth: 400 }}>Select an option from the navigation menu to get started.</p>

              <div style={{ display: "flex", gap: 16, marginTop: 32, flexWrap: "wrap", justifyContent: "center" }}>
                {[
                  { label: "Users", count: users.length, color: "#000000" },
                  { label: "Departments", count: departments.length, color: "#000000" },
                  { label: "Assignments", count: assignments.length, color: "#000000" },
                ].map(s => (
                  <div key={s.label} style={{
                    background: "#fff", borderRadius: 12, padding: "16px 24px",
                    boxShadow: "0 2px 12px rgba(0,0,0,0.06)", minWidth: 110,
                    border: "1px solid rgba(0,0,0,0.10)",
                  }}>
                    <p style={{ fontSize: 28, fontWeight: 500, color: s.color, margin: 0 }}>{s.count}</p>
        ...
        {(activeTab === 'Organization' || activeTab === 'Users' || activeTab === 'Departments') && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, animation: "fadeUp 0.4s ease" }}>
            ...
            {/* Users Management */}
            <div className="card-hover" style={{ background: "#fff", padding: 24, borderRadius: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", display: "flex", flexDirection: "column", border: "1px solid rgba(0,0,0,0.10)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, paddingBottom: 12, borderBottom: "2px solid #e2e2e0", gap: 12 }}>
                <h2 style={{ color: "#000000", fontSize: 17, fontWeight: 500, margin: 0 }}>Manage Users</h2>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ background: "linear-gradient(135deg, #e2e2e0, #ffffff)", color: "#000000", fontSize: 12, padding: "6px 14px", borderRadius: 20, fontWeight: 500 }}>{users.length} Users</span>
                  <button
                    onClick={() => {
                      setIsUserFormOpen(prev => !prev);
                      setUserFormErrors({});
                    }}
                    style={{ background: isUserFormOpen ? "#fff" : "#0A919B", color: isUserFormOpen ? "#0A919B" : "#fff", border: isUserFormOpen ? "1px solid #0A919B" : "none", padding: "7px 14px", borderRadius: 8, cursor: "pointer", fontWeight: 500, fontSize: 12 }}
                  >
              ...
              {isUserFormOpen && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr)) auto", gap: 10, marginBottom: 16, background: "rgba(255,255,255,0.58)", padding: 14, borderRadius: 12, border: "1px solid rgba(0,0,0,0.10)", alignItems: "start" }}>
                <div>
                  ...
                  {userFormErrors.name && <p style={{ margin: "4px 0 0", color: "#ef4444", fontSize: 11, fontWeight: 500 }}>{userFormErrors.name}</p>}
                </div>
                <div>
                  ...
                  {userFormErrors.email && <p style={{ margin: "4px 0 0", color: "#ef4444", fontSize: 11, fontWeight: 500 }}>{userFormErrors.email}</p>}
                </div>
                <div>
                  ...
                  {userFormErrors.form_url && <p style={{ margin: "4px 0 0", color: "#ef4444", fontSize: 11, fontWeight: 500 }}>{userFormErrors.form_url}</p>}
                </div>
                <div>
                  ...
                  {userFormErrors.department_id && <p style={{ margin: "4px 0 0", color: "#ef4444", fontSize: 11, fontWeight: 500 }}>{userFormErrors.department_id}</p>}
                </div>
                <button onClick={createUser} className="btn-primary" style={{ background: "#0A919B", color: "#fff", padding: "8px 18px", borderRadius: 8, border: "none", cursor: "pointer", fontWeight: 500, fontSize: 13, whiteSpace: "nowrap", minHeight: 36 }}>Add User</button>
                {userFormErrors.form && <p style={{ gridColumn: "1 / -1", margin: 0, color: "#ef4444", fontSize: 12, fontWeight: 500 }}>{userFormErrors.form}</p>}
              </div>
              )}
              ...
              <div style={{ overflowX: "auto", flex: 1, maxHeight: 500, overflowY: "auto" }}>
                <table style={{ width: "100%", textAlign: "left", borderCollapse: "collapse" }}>
                  <thead>
                    <tr style={{ background: "rgba(255,255,255,0.58)", borderBottom: "2px solid rgba(0,0,0,0.10)" }}>
                      <th style={{ padding: "10px 14px", fontWeight: 500, fontSize: 12, color: "rgba(0,0,0,0.58)", textTransform: "uppercase", letterSpacing: 0.5 }}>ID</th>
                      <th style={{ padding: "10px 14px", fontWeight: 500, fontSize: 12, color: "rgba(0,0,0,0.58)", textTransform: "uppercase", letterSpacing: 0.5 }}>Name</th>
                      <th style={{ padding: "10px 14px", fontWeight: 500, fontSize: 12, color: "rgba(0,0,0,0.58)", textTransform: "uppercase", letterSpacing: 0.5 }}>Form URL</th>
                      <th style={{ padding: "10px 14px", fontWeight: 500, fontSize: 12, color: "rgba(0,0,0,0.58)", textTransform: "uppercase", letterSpacing: 0.5 }}>Dept</th>
                      <th style={{ padding: "10px 14px", fontWeight: 500, fontSize: 12, color: "rgba(0,0,0,0.58)", textTransform: "uppercase", letterSpacing: 0.5, textAlign: "right" }}>Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {users.length > 0 ? users.map(u => (
                      <tr key={u.user_id} style={{ borderBottom: "1px solid #e2e2e0", transition: "background 0.15s" }}
                        onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.58)"}
                        onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                        <td style={{ padding: "10px 14px", fontSize: 12, color: "rgba(0,0,0,0.42)" }}>{u.user_id}</td>
                        <td style={{ padding: "10px 14px" }}>
                          <p style={{ fontWeight: 500, color: "#000000", margin: 0, fontSize: 13 }}>{u.name}</p>
                          <p style={{ fontSize: 11, color: "rgba(0,0,0,0.42)", margin: 0 }}>{u.email}</p>
                        </td>
                        <td style={{ padding: "10px 14px" }}>
                          {u.form_url ? (
                            <a href={u.form_url} target="_blank" rel="noopener noreferrer" style={{ color: "#0A919B", fontSize: 12, fontWeight: 500, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 4, background: "#e2e2e0", padding: "3px 10px", borderRadius: 6, border: "1px solid #0A919B" }}>
                        ...
                        <td style={{ padding: "10px 14px", fontFamily: "monospace", color: "rgba(0,0,0,0.58)", fontSize: 12 }}>#{u.department_id}</td>
                        <td style={{ padding: "10px 14px", textAlign: "right" }}>
                          <button onClick={() => deleteUser(u.user_id)} style={{ background: "none", border: "none", color: "#ef4444", cursor: "pointer", fontWeight: 500, fontSize: 12 }}>Delete</button>
                        </td>
                      </tr>
                    )) : (
                      <tr><td colSpan="6" style={{ padding: 24, textAlign: "center", color: "rgba(0,0,0,0.42)", fontStyle: "italic" }}>No users found.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            <div className="card-hover" style={{ background: "#fff", padding: 24, borderRadius: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", display: "flex", flexDirection: "column", border: "1px solid rgba(0,0,0,0.10)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, paddingBottom: 12, borderBottom: "2px solid #e2e2e0" }}>
                <h2 style={{ color: "#000000", fontSize: 17, fontWeight: 500, margin: 0 }}>Manage Departments</h2>
                <span style={{ background: "linear-gradient(135deg, #e2e2e0, #ffffff)", color: "#000000", fontSize: 12, padding: "6px 14px", borderRadius: 20, fontWeight: 500 }}>{departments.length} Depts</span>
              </div>
              
              <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 10, marginBottom: 16, background: "rgba(255,255,255,0.58)", padding: 14, borderRadius: 12, border: "1px solid rgba(0,0,0,0.10)", alignItems: "start" }}>
                <div>
                  ...
                  {deptFormErrors.department_name && <p style={{ margin: "4px 0 0", color: "#ef4444", fontSize: 11, fontWeight: 500 }}>{deptFormErrors.department_name}</p>}
                </div>
                <button onClick={createDepartment} className="btn-primary" style={{ background: "#0A919B", color: "#fff", padding: "8px 18px", borderRadius: 8, border: "none", cursor: "pointer", fontWeight: 500, fontSize: 13, whiteSpace: "nowrap", minHeight: 36 }}>Add Dept</button>
                {deptFormErrors.form && <p style={{ gridColumn: "1 / -1", margin: 0, color: "#ef4444", fontSize: 12, fontWeight: 500 }}>{deptFormErrors.form}</p>}
              </div>
              ...
              <div style={{ overflowX: "auto", flex: 1, maxHeight: 500, overflowY: "auto" }}>
                <table style={{ width: "100%", textAlign: "left", borderCollapse: "collapse" }}>
                  <thead>
                    <tr style={{ background: "rgba(255,255,255,0.58)", borderBottom: "2px solid rgba(0,0,0,0.10)" }}>
                      <th style={{ padding: "10px 14px", fontWeight: 500, fontSize: 12, color: "rgba(0,0,0,0.58)", textTransform: "uppercase", letterSpacing: 0.5 }}>ID</th>
                      <th style={{ padding: "10px 14px", fontWeight: 500, fontSize: 12, color: "rgba(0,0,0,0.58)", textTransform: "uppercase", letterSpacing: 0.5 }}>Department Name</th>
                      <th style={{ padding: "10px 14px", fontWeight: 500, fontSize: 12, color: "rgba(0,0,0,0.58)", textTransform: "uppercase", letterSpacing: 0.5, textAlign: "right" }}>Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {departments.length > 0 ? departments.map(d => (
                      <tr key={d.department_id} style={{ borderBottom: "1px solid #e2e2e0", transition: "background 0.15s" }}
                        onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.58)"}
                        onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                        <td style={{ padding: "10px 14px", fontWeight: 500, color: "rgba(0,0,0,0.42)", fontSize: 12 }}>#{d.department_id}</td>
                        <td style={{ padding: "10px 14px", fontWeight: 500, color: "#000000", fontSize: 13 }}>{d.department_name}</td>
                        <td style={{ padding: "10px 14px", textAlign: "right" }}>
                          <button onClick={() => deleteDepartment(d.department_id)} style={{ background: "none", border: "none", color: "#ef4444", cursor: "pointer", fontWeight: 500, fontSize: 12 }}>Delete</button>
                        </td>
                      </tr>
                    )) : (
                      <tr><td colSpan="3" style={{ padding: 24, textAlign: "center", color: "rgba(0,0,0,0.42)", fontStyle: "italic" }}>No departments found.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20, animation: "fadeUp 0.4s ease" }}>
          {/* Assignment Controls */}
          <div className="card-hover" style={{ background: "#fff", padding: 24, borderRadius: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", border: "1px solid rgba(0,0,0,0.10)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, paddingBottom: 12, borderBottom: "2px solid #e2e2e0" }}>
              <h2 style={{ color: "#000000", fontSize: 17, fontWeight: 500, margin: 0 }}>Filter Assignments</h2>
            </div>
            
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              <div style={{ display: "flex", flexDirection: "column" }}>
                <label style={{ fontSize: 12, fontWeight: 500, color: "rgba(0,0,0,0.58)", marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}>Filter by Reviewer</label>
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                <label style={{ fontSize: 12, fontWeight: 500, color: "rgba(0,0,0,0.58)", marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}>Filter by Reviewee</label>
          ...
          <div className="card-hover" style={{ background: "#fff", padding: 24, borderRadius: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", display: "flex", flexDirection: "column", border: "1px solid rgba(0,0,0,0.10)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, paddingBottom: 12, borderBottom: "2px solid #e2e2e0", flexWrap: "wrap", gap: 8 }}>
              <h2 style={{ color: "#000000", fontSize: 17, fontWeight: 500, margin: 0 }}>Review Assignments Hub</h2>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, background: "rgba(255,255,255,0.58)", border: "1px solid rgba(0,0,0,0.10)", padding: "6px 12px", borderRadius: 8 }}>
                  <label style={{ color: "rgba(0,0,0,0.58)", fontWeight: 500, margin: 0, whiteSpace: "nowrap" }}>Batch:</label>
                  <select
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, background: "rgba(255,255,255,0.58)", border: "1px solid rgba(0,0,0,0.10)", padding: "6px 12px", borderRadius: 8 }}>
                  <label style={{ color: "rgba(0,0,0,0.58)", fontWeight: 500, margin: 0, whiteSpace: "nowrap" }}>Per Person:</label>
                  <input 
                    type="number" min="1" max={maxAssignmentsPerUser}
                ...
                <button
                  ...
                  style={{
                    background: (!selectedBatchId || isBatchSending) ? "rgba(0,0,0,0.42)" : "#0A919B",
                    color: "#fff",
                    padding: "8px 16px",
                    borderRadius: 8,
                    border: "none",
            ...
            {selectedBatchId && batches.find(b => String(b.id) === String(selectedBatchId)) && (
              <div style={{ background: "linear-gradient(135deg, #e2e2e0, #ffffff)", borderLeft: "4px solid #0A919B", padding: "10px 16px", borderRadius: "0 8px 8px 0", marginBottom: 14, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                <p style={{ margin: 0, color: "#000000", fontWeight: 500, fontSize: 13 }}>
                  Showing batch: <strong>{batches.find(b => String(b.id) === String(selectedBatchId))?.label || selectedBatchId}</strong>
                </p>
                <span style={{ background: "#0A919B", color: "#fff", fontSize: 11, padding: "3px 10px", borderRadius: 12, fontWeight: 500 }}>
            ...
            {editingReviewerId && (
              <div style={{ border: "1px solid #0A919B", background: "#f8fdff", borderRadius: 12, padding: 16, marginBottom: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "center", marginBottom: 12, flexWrap: "wrap"}}>
                  <div>
                    <p style={{ margin: 0, color: "#000000", fontWeight: 500, fontSize: 14 }}>Editing {getUserName(editingReviewerId)}</p>
                    <p style={{ margin: "3px 0 0", color: "rgba(0,0,0,0.58)", fontSize: 12 }}>Select the people this reviewer should review in the selected batch.</p>
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={cancelEditAssignments} style={{ background: "#fff", border: "1px solid #d8e2ea", color: "#000000", padding: "8px 14px", borderRadius: 8, cursor: "pointer", fontWeight: 500, fontSize: 12 }}>Cancel</button>
                    <button onClick={saveReviewerAssignments} disabled={isSavingAssignments} style={{ background: isSavingAssignments ? "rgba(0,0,0,0.42)" : "#0A919B", border: "none", color: "#fff", padding: "8px 16px", borderRadius: 8, cursor: isSavingAssignments ? "not-allowed" : "pointer", fontWeight: 500, fontSize: 12 }}>
                      {isSavingAssignments ? "Saving..." : "Save List"}
                    </button>
                  </div>
                </div>

                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 8, maxHeight: 260, overflowY: "auto", paddingRight: 4 }}>
                  {users.map(user => {
                    const disabled = String(user.user_id) === String(editingReviewerId);
                    const selected = editRevieweeIds.includes(user.user_id);
                    return (
                      <button
                        key={user.user_id}
                        onClick={() => !disabled && toggleEditReviewee(user.user_id)}
                        disabled={disabled}
                        style={{
                          textAlign: "left",
                          display: "flex",
                          alignItems: "center",
                          gap: 10,
                          border: selected ? "1px solid #0A919B" : "1px solid rgba(0,0,0,0.10)",
                          background: disabled ? "#f1f5f9" : selected ? "#ecfdf5" : "#fff",
                      ...
                      >
                        <span style={{ width: 22, height: 22, borderRadius: 6, border: selected ? "none" : "1px solid #cbd5e0", background: selected ? "#0A919B" : "#fff", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 500 }}>
                          {selected ? <CheckIcon size={13} /> : ""}
                        </span>
                        <span style={{ minWidth: 0 }}>
                          <span style={{ display: "block", color: "#000000", fontWeight: 500, fontSize: 12, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user.name}</span>
                          <span style={{ display: "block", color: "rgba(0,0,0,0.45)", fontSize: 11, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{getUserDepartmentName(user.user_id)}</span>
                        </span>
                      </button>
                    );
                  })}
            ...
            <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "2px 8px 2px 0", maxHeight: 620, display: "flex", flexDirection: "column", gap: 14 }}>
              {assignmentGroups.length > 0 ? (
                ...
                <div style={{ padding: 36, textAlign: "center", color: "rgba(0,0,0,0.45)", background: "rgba(255,255,255,0.58)", border: "1px dashed #d8e2ea", borderRadius: 12, fontStyle: "italic" }}>
        ...
        {activeTab === 'Manual Assign' && (
          <div style={{ animation: "fadeUp 0.4s ease" }}>
                ...
                <React.Fragment key={step.num}>
                  <div
                    ...
                    style={{
                      ...
                      background: manualStep === step.num ? "#0A919B" : manualStep > step.num ? "#ffffff" : "#e2e2e0",
                      color: manualStep === step.num ? "#fff" : manualStep > step.num ? "#000000" : "rgba(0,0,0,0.42)",
                      transition: "all 0.3s ease",
                    }}
                  >
                    <span style={{
                      width: 28, height: 28, borderRadius: "50%",
                      background: manualStep === step.num ? "rgba(255,255,255,0.25)" : manualStep > step.num ? "#000000" : "#e2e8f0",
                      color: manualStep > step.num ? "#fff" : "inherit",
                      display: "flex", alignItems: "center", justifyContent: "center",
                      fontWeight: 500, fontSize: 13,
                    }}>
                      {manualStep > step.num ? <CheckIcon size={14} /> : step.num}
                    </span>
                    <span style={{ fontWeight: 500, fontSize: 13 }}>{step.label}</span>
                  </div>
                  {i < 2 && (
                    <div style={{ width: 48, height: 2, background: manualStep > step.num ? "#000000" : "#e2e8f0", transition: "background 0.3s" }} />
                  )}
                </React.Fragment>
              ))}
            </div>
            ...
            {manualStep === 1 && (
              <div className="card-hover" style={{ background: "#fff", borderRadius: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", border: "1px solid rgba(0,0,0,0.10)", overflow: "hidden" }}>
                <div style={{ padding: "20px 24px", borderBottom: "2px solid #e2e2e0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <h2 style={{ color: "#000000", fontSize: 17, fontWeight: 500, margin: 0 }}>Step 1: Select Reviewer</h2>
                    <p style={{ color: "rgba(0,0,0,0.58)", fontSize: 13, margin: "4px 0 0" }}>Choose who will receive the assignment email</p>
                  </div>
                  <span style={{ background: "linear-gradient(135deg, #e2e2e0, #ffffff)", color: "#000000", fontSize: 12, padding: "6px 14px", borderRadius: 20, fontWeight: 500 }}>
                    {manualReviewer.length} selected
                  </span>
                </div>

                <div style={{ padding: "16px 24px", borderBottom: "1px solid #e2e2e0", display: "flex", gap: 12, alignItems: "center" }}>
                  <div style={{ position: "relative", flex: 1 }}>
                    <svg style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "rgba(0,0,0,0.42)" }} width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <button onClick={selectAllReviewer} style={{
                    background: "#e2e2e0", border: "1px solid #e2e8f0", padding: "10px 16px", borderRadius: 10,
                    cursor: "pointer", fontWeight: 500, fontSize: 12, color: "#000000", whiteSpace: "nowrap", transition: "all 0.2s",
                  }}>
                    {(() => {
                      const eligible = users.filter(u => u.name.toLowerCase().includes(Reviewerearch.toLowerCase()) && !manualReviewee.includes(u.user_id));
                      return eligible.length > 0 && eligible.every(u => manualReviewer.includes(u.user_id)) ? "Deselect All" : "Select All";
                    })()}
                  </button>
                </div>
                ...
                <div style={{ maxHeight: 400, overflowY: "auto", padding: "8px 12px" }}>
                  {users.filter(u => u.name.toLowerCase().includes(Reviewerearch.toLowerCase())).map(u => {
                    ...
                    const isBlocked = manualReviewee.includes(u.user_id);
                    return (
                      <div
                        key={u.user_id}
                        ...
                        style={{
                          ...
                          background: isBlocked ? "#f8fafc" : isSelected ? "#e2e2e0" : "transparent",
                          border: isSelected ? "1px solid #0A919B" : "1px solid transparent",
                      ...
                      >
                        <div style={{
                          ...
                          background: isSelected ? "#0A919B" : isBlocked ? "#f1f5f9" : "#fff",
                          display: "flex", alignItems: "center", justifyContent: "center",
                        ...
                        <div style={{
                          ...
                          background: isSelected ? "#0A919B" : "linear-gradient(135deg, #e2e8f0, #cbd5e0)",
                          display: "flex", alignItems: "center", justifyContent: "center",
                          color: isSelected ? "#fff" : "rgba(0,0,0,0.58)", fontWeight: 500, fontSize: 14, flexShrink: 0,
                        }}>
                          {u.name.charAt(0)}
                        </div>
                        <div style={{ flex: 1 }}>
                        ...
                        <span style={{ background: "rgba(0,0,0,0.06)", color: isBlocked ? "rgba(0,0,0,0.58)" : "#0A919B", padding: "3px 10px", borderRadius: 6, fontWeight: 500, fontSize: 11 }}>
                          {isBlocked ? "Selected as reviewee" : getUserDepartmentName(u.user_id)}
                        </span>
                      </div>
                    );
                  })}
                ...
                <div style={{ padding: "16px 24px", borderTop: "1px solid #e2e2e0", display: "flex", justifyContent: "flex-end" }}>
                  <button
                    onClick={() => { if (manualReviewer.length > 0) setManualStep(2); else showToast("Select at least one recipient", "error"); }}
                    className="btn-primary"
                    style={{
                      background: manualReviewer.length > 0 ? "#0A919B" : "#e2e8f0",
                      color: manualReviewer.length > 0 ? "#fff" : "rgba(0,0,0,0.42)",
            ...
            {/* Step 2: Select Reviewee */}
            {manualStep === 2 && (
              <div className="card-hover" style={{ background: "#fff", borderRadius: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", border: "1px solid rgba(0,0,0,0.10)", overflow: "hidden" }}>
                <div style={{ padding: "20px 24px", borderBottom: "2px solid #e2e2e0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <h2 style={{ color: "#000000", fontSize: 17, fontWeight: 500, margin: 0 }}>Step 2: Select Users to Assign</h2>
                    <p style={{ color: "rgba(0,0,0,0.58)", fontSize: 13, margin: "4px 0 0" }}>Choose the users that Reviewer will need to review</p>
                  </div>
                  <span style={{ background: "linear-gradient(135deg, rgba(0,0,0,0.06), rgba(0,0,0,0.10))", color: "#000000", fontSize: 12, padding: "6px 14px", borderRadius: 20, fontWeight: 500 }}>
                    {manualReviewee.length} selected
                  </span>
                </div>

                {/* Selected Reviewer preview */}
                <div style={{ padding: "12px 24px", background: "rgba(255,255,255,0.58)", borderBottom: "1px solid rgba(0,0,0,0.10)", display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 12, fontWeight: 500, color: "rgba(0,0,0,0.58)" }}>Reviewer:</span>
                  {manualReviewer.map(id => (
                    <span key={id} style={{
                      background: "linear-gradient(135deg, #e2e2e0, #ffffff)", color: "#000000",
                      padding: "4px 12px", borderRadius: 16, fontSize: 11, fontWeight: 500,
                      display: "flex", alignItems: "center", gap: 6,
                    }}>
                      {getUserName(id)}
                      <button onClick={() => toggleRecipient(id)} style={{ background: "none", border: "none", color: "#000000", cursor: "pointer", fontSize: 14, padding: 0, lineHeight: 1 }}>x</button>
                    </span>
                  ))}
                </div>
                ...
                <div style={{ padding: "16px 24px", borderBottom: "1px solid #e2e2e0", display: "flex", gap: 12, alignItems: "center" }}>
                  ...
                  <button onClick={selectAllAssignees} style={{
                    ...
                    {(() => {
                      const eligible = users.filter(u => u.name.toLowerCase().includes(assigneeSearch.toLowerCase()) && !manualReviewer.includes(u.user_id));
                      return eligible.length > 0 && eligible.every(u => manualReviewee.includes(u.user_id)) ? "Deselect All" : "Select All";
                    })()}
                  </button>
                </div>
                ...
                <div style={{ maxHeight: 400, overflowY: "auto", padding: "8px 12px" }}>
                  {users.filter(u => u.name.toLowerCase().includes(assigneeSearch.toLowerCase())).map(u => {
                    ...
                    const isBlocked = manualReviewer.includes(u.user_id);
                    return (
                      <div
                        key={u.user_id}
                        className="check-item"
                        ...
                        style={{
                          display: "flex", alignItems: "center", gap: 12,
                          padding: "10px 14px", borderRadius: 10, marginBottom: 2,
                          background: isBlocked ? "#f8fafc" : isSelected ? "rgba(0,0,0,0.06)" : "transparent",
                          border: isSelected ? "1px solid rgba(32,38,38,0.12)" : "1px solid transparent",
                          cursor: isBlocked ? "not-allowed" : "pointer",
                          opacity: isBlocked ? 0.55 : 1,
                        }}
                      >
                        ...
                        <span style={{ background: "rgba(0,0,0,0.06)", color: isBlocked ? "rgba(0,0,0,0.58)" : "#0A919B", padding: "3px 10px", borderRadius: 6, fontWeight: 500, fontSize: 11 }}>
                          {isBlocked ? "Selected as reviewer" : getUserDepartmentName(u.user_id)}
                        </span>
                      </div>
                    );
                  })}
                ...
                <div style={{ padding: "16px 24px", borderTop: "1px solid #e2e2e0", display: "flex", justifyContent: "space-between" }}>
                  <button onClick={() => setManualStep(1)} style={{
                    background: "#e2e2e0", color: "#000000", padding: "10px 28px", borderRadius: 10,
                    border: "1px solid #e2e8f0", cursor: "pointer", fontWeight: 500, fontSize: 14,
                  ...
                  <button
                    ...
                    className="btn-primary"
                    style={{
                      background: manualReviewee.length > 0 ? "#0A919B" : "#e2e8f0",
                      color: manualReviewee.length > 0 ? "#fff" : "rgba(0,0,0,0.42)",
                      padding: "10px 28px", borderRadius: 10, border: "none", cursor: "pointer",
                      fontWeight: 500, fontSize: 14, display: "flex", alignItems: "center", gap: 8,
                    }}
                  >
            ...
            {manualStep === 3 && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                {/* Reviewer Card */}
                <div className="card-hover" style={{ background: "#fff", borderRadius: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", border: "1px solid rgba(0,0,0,0.10)", overflow: "hidden" }}>
                  <div style={{ padding: "16px 24px", borderBottom: "2px solid #e2e2e0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <h3 style={{ color: "#000000", fontSize: 15, fontWeight: 500, margin: 0 }}>Reviewer</h3>
                    <span style={{ background: "linear-gradient(135deg, #e2e2e0, #ffffff)", color: "#000000", fontSize: 11, padding: "4px 12px", borderRadius: 16, fontWeight: 500 }}>{manualReviewer.length}</span>
                  </div>
                  <div style={{ padding: "12px 16px", maxHeight: 300, overflowY: "auto" }}>
                    {manualReviewer.map(id => (
                      <div key={id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", borderRadius: 8, marginBottom: 4 }}
                        onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.58)"}
                        onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                        <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#0A919B", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 500, fontSize: 13, flexShrink: 0 }}>
                          {getUserName(id).charAt(0)}
                        </div>
                        <div>
                          <p style={{ fontWeight: 500, color: "#000000", margin: 0, fontSize: 13 }}>{getUserName(id)}</p>
                          <p style={{ fontSize: 11, color: "rgba(0,0,0,0.42)", margin: 0 }}>{getUserEmail(id)}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="card-hover" style={{ background: "#fff", borderRadius: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", border: "1px solid rgba(0,0,0,0.10)", overflow: "hidden" }}>
                  <div style={{ padding: "16px 24px", borderBottom: "2px solid #e2e2e0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <h3 style={{ color: "#000000", fontSize: 15, fontWeight: 500, margin: 0 }}>Assigned Users</h3>
                    <span style={{ background: "linear-gradient(135deg, rgba(0,0,0,0.06), rgba(0,0,0,0.10))", color: "#000000", fontSize: 11, padding: "4px 12px", borderRadius: 16, fontWeight: 500 }}>{manualReviewee.length}</span>
                  </div>
                  <div style={{ padding: "12px 16px", maxHeight: 300, overflowY: "auto" }}>
                ...
                <div style={{ gridColumn: "1 / -1" }}>
                  <div style={{
                    background: "linear-gradient(135deg, #ffffff, #e2e2e0)",
                    border: "1px solid #0A919B", borderRadius: 16, padding: 24,
                    display: "flex", justifyContent: "space-between", alignItems: "center",
                  }}>
                    <div>
                      <p style={{ fontWeight: 500, color: "#000000", fontSize: 15, margin: 0 }}>Ready to send</p>
                      <p style={{ color: "#000000", fontSize: 13, margin: "4px 0 0" }}>
                    ...
                    <div style={{ display: "flex", gap: 12 }}>
                      <button onClick={() => setManualStep(2)} style={{
                        background: "#fff", color: "#000000", padding: "12px 24px", borderRadius: 10,
                        border: "1px solid #e2e8f0", cursor: "pointer", fontWeight: 500, fontSize: 13,
                      }}>
                        Back
                      </button>
                      <button
                        ...
                        className="btn-primary"
                        style={{
                          background: isSending ? "rgba(0,0,0,0.42)" : "#0A919B",
